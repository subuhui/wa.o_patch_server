...
### 1. Release 多平台发布
- 创建与查询 Release 时，服务端根据 `release_artifacts` 动态统计各平台的已发布产物状态（`android`, `ios`）。
- 支持同一版本号（如 `1.0.0+2`）跨平台分步发布（例如先发布 Android 底包，后续再补充上传 iOS 底包与符号产物），避免平台误判冲突。
...
5. 客户端事件追踪：支持接收并记录客户端上报的补丁下载（`__patch_download__`）与安装（`__patch_install__`）生命周期事件。
...
支持应用改名、删除自定义渠道和恢复已回滚补丁。内置渠道 stable、beta、staging 不允许删除；删除自定义渠道会解除补丁的渠道关联，保留补丁记录。恢复补丁保留原渠道，首次恢复返回 204，已处于 active 状态时返回 304；草稿或失去渠道关联的补丁不能恢复。
